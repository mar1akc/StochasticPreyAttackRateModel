...
// compile command: gcc expOUmodel_mean_covar.c -lm -O3
...
// #define VARIANCE 1.25e-3  
#define SIGMA 0.01

#define Nvals_g 31
#define Nvals_alpha 5
...
double DELTA, ALPHA; //, SIGMA;
double mean0, mean1, cov00, cov01, cov11;
double mean3_0,mean3_1,mean3_2,cov3_00,cov3_01,cov3_02,cov3_11,cov3_12,cov3_22;
//------------------------------
...
void binning_trajectory(long Nsteps,long *bins,long *ubins,double *f,double *pop,
	...
	double *w, *u;
	double dt = TIME_STEP, sqrt_dt = sqrt(TIME_STEP);
	double delta0, delta1, delta0_new, delta1_new;
	double delta3_0, delta3_1, delta3_2, delta3_0_new, delta3_1_new, delta3_2_new;
	long js1;   // sample count after including this point
...
// 			printf("j1 = %i, aux = %i\n",j1,(int)floor(((*u) - (*umin))/(*ustep)+0.5));
			...
			js1 = jstep + 1;
			// running covariance for pop[0] vs (pop[1]+pop[2])
			delta0 = pop[0] - mean0;             // deviation from OLD mean
			delta1 = (pop[1] + pop[2]) - mean1;
			mean0 += delta0 / js1;                  // update means
			mean1 += delta1 / js1;
			delta0_new = pop[0] - mean0;          // deviation from NEW mean
			delta1_new = (pop[1] + pop[2]) - mean1;
			cov00 += delta0 * delta0_new;         // accumulate M2-style sums (NOT averages yet)
			cov01 += delta0 * delta1_new;
			cov11 += delta1 * delta1_new;

			// running 3x3 covariance for pop[0], pop[1], pop[2] individually
			delta3_0 = pop[0] - mean3_0;          // deviations from OLD means
			delta3_1 = pop[1] - mean3_1;
			delta3_2 = pop[2] - mean3_2;
			mean3_0 += delta3_0 / js1;               // update means
			mean3_1 += delta3_1 / js1;
			mean3_2 += delta3_2 / js1;
			delta3_0_new = pop[0] - mean3_0;      // deviations from NEW means
			delta3_1_new = pop[1] - mean3_1;
			delta3_2_new = pop[2] - mean3_2;
			cov3_00 += delta3_0 * delta3_0_new;   // accumulate M2-style sums
			cov3_01 += delta3_0 * delta3_1_new;
			cov3_02 += delta3_0 * delta3_2_new;
			cov3_11 += delta3_1 * delta3_1_new;
			cov3_12 += delta3_1 * delta3_2_new;
			cov3_22 += delta3_2 * delta3_2_new;
...
void driver(int run) {
	...
	*ustep = ((*umax) - (*umin))/(N_BIN - 1);
	...
	sprintf(fname,"HopfShiftData_1/bins_alpha%.3f_sigma%.3f_gbar%.4f.txt",ALPHA,SIGMA,gbar);
	fid = fopen(fname,"w");
	for( i=0; i<N_BIN; i++ ) {
		for( j=0; j<N_BIN; j++ ) {
			ind = i + j*N_BIN;
	...
	sprintf(fname,"HopfShiftData_1/ubins_alpha%.3f_sigma%.3f.txt",ALPHA,SIGMA);
	fid = fopen(fname,"w");
	...
	sprintf(fname,"HopfShiftData_1/params_run%i.txt",run);
	fid = fopen(fname,"w");
...
int main() {
	const double gmax = 0.46, gmin = 0.43;
	const double alpha_max = 4., alpha_min = 0.02;
	int g_index, alpha_index;
	double  g_vals[Nvals_g], alpha_vals[] = {0.04, 0.1,  0.4,  1., 4.  };
	double g_step, alpha_step;
	int run, j_g, j_alpha;

	g_step = (gmax - gmin)/(Nvals_g - 1);
// 	alpha_step = (alpha_max - alpha_min)/(Nvals - 1);
	for( j_g = 0; j_g < Nvals_g; j_g++ ) {
		g_vals[j_g] = gmin + j_g*g_step;
// 		alpha_vals[j_g] = alpha_min + j_g*alpha_step;
	}
	
	run = 0;
	FILE *ff,*f3;
	
	ff = fopen("HopfShiftData_1/mean_cov.txt","w");
	f3 = fopen("HopfShiftData_1/mean_cov3.txt","w");
	for( j_g = 0; j_g < Nvals_g; j_g++ ){
		DELTA = g_vals[j_g] - GMIN;
		for( j_alpha = 0; j_alpha < Nvals_alpha; j_alpha++ ) {
			ALPHA = alpha_vals[j_alpha];
// 			SIGMA = sqrt(2.0*ALPHA*VARIANCE);
			driver(run);
			printf("Finished run %i: ALPHA = %.4f, G = %.4f, SIGMA = %.4e\n",run,ALPHA,g_vals[j_g],SIGMA);
			fprintf(ff,"%i\t%.4f\t%.4f\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\n",run,ALPHA,g_vals[j_g],mean0,mean1,cov00,cov01,cov11);
			printf("mean = (%.4e\t%.4e)\t cov = (%.4e\t%.4e\t%.4e)\n",mean0,mean1,cov00,cov01,cov11);
			fprintf(f3,"%i\t%.4f\t%.4f\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\t%.4e\n",
				run,ALPHA,g_vals[j_g],mean3_0,mean3_1,mean3_2,cov3_00,cov3_01,cov3_02,cov3_11,cov3_12,cov3_22);
			run++;		
		}	
	}
	fclose(ff);
